...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        if (invalidItem) {
            setCartError(
                `${invalidItem.name} is ${
                    !invalidItem.is_active ? "Not available" : "out of stock"
                    ...
                    {cart.length > 0 && (
                        <>
                            ...
                            {cart.map((item, index) => {
                                const offerPrice =
                                    item.price -
                                    (item.price * item.discount_percentage) /
                                        100;
                                return (
                                    <div
                                        key={index}
                ...
                <p className="text-base-content/60 text-justify mb-4">You're one step away from getting these items delivered to your doorstep. Double-check your cart and enjoy a smooth and secure checkout experience. We're excited to deliver happiness your way!</p>
                    ...
                    <button
                        ...
                        className={`w-full py-3 mt-6 font-medium transition ${
                            cartError
                                ? "bg-error/40 text-error-content cursor-not-allowed"
                                : "bg-primary/80 text-base-content hover:bg-primary/70 cursor-pointer"
                        }`}
                    >
